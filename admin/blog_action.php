  ...
  $i=0;
   ...
   ';
    ...
    if($_POST["action"] == "insert")
    {
        ...
        $date=date("F d, Y");
        $query = "INSERT INTO $tablename(`title`,`category`,`description`,`image`,`date`)  VALUES ('$title','$category','$description','$file','$date')";
    ...
    if($_POST["action"] == "update")
    {
        ...
        $description = mysqli_real_escape_string($connect,$_POST['description']);
        $file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
        $query = "UPDATE $tablename SET `image` = '$file',`title` = '$title',`category` = '$category' , `description`='$description' WHERE id = '".$_POST["image_id"]."'";
        if(mysqli_query($connect, $query))
        {
            echo 'Data Updated!';
        }
