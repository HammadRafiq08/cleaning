  ...
  $i=0;
        while($row = mysqli_fetch_array($result))
        {
            ...
            if(strlen($row["description"])>200)
            {
                ...
                $description=substr($row["description"],0,strlen($row["description"]));
            }
   ...
   ';
    ...
    if($_POST["action"] == "insert")
    {
        ...
        $safety = mysqli_real_escape_string($connect,$_POST['safety']);
        $file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
        $query = "INSERT INTO $tablename(`name`,`description`,`image`,`safety`)  VALUES ('$name','$description','$file','$safety')";
    ...
    if($_POST["action"] == "update")
    {
        ...
        $file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
        $safety = mysqli_real_escape_string($connect,$_POST['safety']);
        $query = "UPDATE $tablename SET `image` = '$file',`name` = '$name', `safety`='$safety', `description`='$description' WHERE id = '".$_POST["image_id"]."'";
        if(mysqli_query($connect, $query))
        {
            echo 'Data Updated!';
        }
