  ...
  $i=0;
   ...
   ';
    ...
    if($_POST["action"] == "insert")
    {
        ...
        $query = "INSERT INTO $tablename(`name`,`description`,`image`,`category`)  VALUES ('$name','$description','$file','$category')";
    ...
    if($_POST["action"] == "update")
    {
        ...
        $description = mysqli_real_escape_string($connect,$_POST['description']);
        $file = addslashes(file_get_contents($_FILES["image"]["tmp_name"]));
        $query = "UPDATE $tablename SET `image` = '$file',`name` = '$name', `description`='$description',`category`='$category' WHERE id = '".$_POST["image_id"]."'";
        if(mysqli_query($connect, $query))
        {
            echo 'Data Updated!';
        }
